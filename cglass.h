...
namespace ObjectiveGlass {        //the objective glass start.
	...
	public:      //Public characteristics.
		...
		void DrawFloating(double x, double y, double x1, double y1, char*Color = "Red", unsigned int LineThickness = 1)
		{
			//static bool FirstCall = false;
			double totalLenght = 0;
				lx1 = x;
	...
	public:
		...
		void DrawFloating(double x, double y, double radius, char* color = "Red", char* state = "Filled", unsigned int LineThickness = 1)    // Function to draw the Circle.
		{
                origin_x = x; origin_y = y; givenRadius = radius;
				givenColor = color; givenState = state; givenThickness = LineThickness;
			    CreateCircle(origin_x, origin_y, givenRadius, givenColor, givenState, givenThickness);
		}    // END draw function. It draws the circle that does can do things out side the scope of the 
	...
	private:
		...
		bool ObjectAngle = 0;
		void Maker1()            //Makes the rectangle
		{
		...
		void reRotate(double angle) {                               //Function to rotate the rectangle as well as square.
			double radian = 1 * angle * M_PI / 180;
			double pangle = atan((givenHeight / 2) / (givenWidth / 2)) * 180 / M_PI;
			double multi = pangle / 180;
	...
	public:
		...
		void Rotate(double inShape_Axis_x, double inShape_Axis_y, double angle)   //function to rotate the shape around the cerain axis
		{

			double radian = angle * M_PI / 180;
			static double thetaULR = 0, thetaURR = 0, thetaLLR = 0, thetaLRR = 0, thetaOAR = 0;
			static double ulr, urr, llr, lrr, oar;
		
			if (RotateCall == false) {
				axis_X = inShape_Axis_x;
				axis_Y = inShape_Axis_y;

				//distances...
                ulr = sqrt(pow(axis_X - UpperLeft_x, 2) + pow(axis_Y - UpperLeft_y, 2));
				urr = sqrt(pow(axis_X - UpperRight_x, 2) + pow(axis_Y - UpperRight_y, 2));
				llr = sqrt(pow(axis_X - LowerLeft_x, 2) + pow(axis_Y - LowerLeft_y, 2));
				lrr = sqrt(pow(axis_X - LowerRight_x, 2) + pow(axis_Y - LowerRight_y, 2));
				oar = sqrt(pow(axis_X - origin_x, 2) + pow(axis_Y - origin_y, 2));
				//distances...
				//angles
				 
				if (ulr != 0)
				{
					thetaULR = -1*atan((UpperLeft_y - axis_Y) / (UpperLeft_x - axis_X));
					if (!(axis_X == UpperLeft_x || axis_Y == UpperLeft_y)) {      // FOR THE AXIS DIRECTLY OPPSITE TO THE GIVEN AXIS
						thetaULR = -1 * thetaULR;
					}
				}
				else;
				if (urr != 0)
				{
					thetaURR = -1*atan((UpperRight_y - axis_Y) / (UpperRight_x - axis_X));
				}
				else;
				if (llr != 0)
				{
					thetaLLR = -1*atan((LowerLeft_y - axis_Y) / (LowerLeft_x - axis_X));
					if (!(axis_X == LowerLeft_x || axis_Y == LowerLeft_y))
					{
						thetaLLR = -1 * thetaLLR;
					}
				}
				...
				if (lrr != 0)
				{
					thetaLRR = atan((LowerRight_y - axis_Y) / (LowerRight_x - axis_X));
				}
				if (oar != 0)
				{
					thetaOAR = atan((origin_y - axis_Y) / (origin_x - axis_X));
				}
				RotateCall = true;
			}

			UpperLeft_x = axis_X  + ulr * cos(M_PI+thetaULR-radian);      //Calculation of translated axes
			UpperLeft_y = axis_Y  + ulr * sin(M_PI-radian+thetaULR);
			UpperRight_x = axis_X + urr * cos( thetaURR + radian);
			UpperRight_y = axis_Y + urr * sin(M_PI+ thetaURR + radian);
			LowerRight_x = axis_X + lrr * cos(thetaLRR - radian);
			LowerRight_y = axis_Y + lrr * sin(thetaLRR - radian);
			LowerLeft_x = axis_X + llr * cos(M_PI+thetaLLR - radian);
			LowerLeft_y = axis_Y + llr * sin(M_PI+thetaLLR - radian);
			origin_x = axis_X + oar* cos(thetaOAR - radian);
			origin_y = axis_Y + oar* sin(thetaOAR - radian);
           
		    }

		void Revolve(double outShape_Axis_x, double outShape_Axis_y, double Revolve_Angle, double Spin_Angle)   //function to rotate the shape around the cerain axis
		{

			double radian = Revolve_Angle * M_PI / 180;
			radian = M_PI + radian;
			static double thetaULR = 0, thetaURR = 0, thetaLLR = 0, thetaLRR = 0, thetaOAR = 0;
			static double ulr, urr, llr, lrr, oar;

			if (RotateCall == false) {
				axis_X = outShape_Axis_x;
				axis_Y = outShape_Axis_y;
			...
			UpperLeft_x = axis_X + ulr * cos(M_PI + thetaULR - radian);      //Calculation of translated axes
			UpperLeft_y = axis_Y + ulr * sin(M_PI - radian + thetaULR);
			UpperRight_x = axis_X + urr * cos(M_PI+thetaURR + radian);
			UpperRight_y = axis_Y + urr * sin( thetaURR + radian);
			LowerRight_x = axis_X + lrr * cos(M_PI+thetaLRR - radian);
			LowerRight_y = axis_Y + lrr * sin(M_PI+thetaLRR - radian);
			LowerLeft_x = axis_X + llr * cos(M_PI + thetaLLR - radian);
			LowerLeft_y = axis_Y + llr * sin(M_PI + thetaLLR - radian);
			origin_x = axis_X + oar* cos(thetaOAR - radian);
			origin_y = axis_Y + oar* sin(thetaOAR - radian);
		...
		void Revolve(double outShape_Axis_x, double outShape_Axis_y, double Revolve_Angle)   //function to rotate the shape around the cerain axis
		{
			double radian = Revolve_Angle * M_PI / 180;
			static double thetaULR = 0, thetaURR = 0, thetaLLR = 0, thetaLRR = 0, thetaOAR = 0;
			static double ulr, urr, llr, lrr, oar;
